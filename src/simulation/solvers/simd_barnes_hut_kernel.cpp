...
Vec3 accelerationFromNode(const BarnesHutTree& tree, std::size_t node_index, std::size_t target,
    ...
    const BarnesHutTree::NodeView node = tree.node(node_index);
    ...
    if (node.child_count != 0 && !target_inside && distance > 0.0
        ...
        return (node.center_of_mass - target_position)
    ...
    for (std::size_t child = 0; child < node.child_count; ++child) {
        acceleration += accelerationFromNode(tree, node.children[child], target, target_position, world,
                                             gravitational_constant, softening_squared, opening_angle);
    }
    return acceleration;
}
